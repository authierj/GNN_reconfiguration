    ...
    with open(experiment_filepath[0]) as exp_file:
        while (line := exp_file.readline().rstrip()):
            if not flag_start:
                ...
                if not os.path.exists(exp_filepath) and not os.path.exists(exp_filepath_small):
                    print('Error: experiment line {} not found'.format(exp_counter))
                else:
                    exp_filepath_get = exp_filepath if os.path.exists(exp_filepath) else exp_filepath_small
                    with open(exp_filepath_get, 'rb') as exp_handle:
                        exp_nn = line[line.find('hidden_features: ') :line.find(', run')]
...
def plot_exp_NNsize(exp_stats, run_counter, current_nn, exp_counter):
    ...
    plt.plot(exp_stats['T_loss'] / run_counter, label='NN-' + current_nn, color=f"C{exp_counter}")
    plt.figure(2)  # valid loss
    plt.yscale("log")
    plt.plot(exp_stats['V_loss'] / run_counter, label='NN-' + current_nn, color=f"C{exp_counter}")
    plt.figure(3)  # T & V dispatch error, log scale
    plt.yscale("log")
    plt.plot(exp_stats['T_dispatch_mean'] / run_counter, label='NN-' + current_nn, color=f"C{exp_counter}")
    plt.plot(exp_stats['V_dispatch_mean'] / run_counter, "--", color=f"C{exp_counter}") #  + '-V'
    plt.figure(4)  # T & V topology error
    plt.plot(exp_stats['T_topology_mean'] / run_counter, label='NN-' + current_nn, color=f"C{exp_counter}")
    plt.plot(exp_stats['V_topology_mean'] / run_counter, "--", color=f"C{exp_counter}") # + '-V'
    # plt.figure(5)  # V ineq error violation
...
def parse_pareto_datasize_NN(exp_filepaths):
    ...
    for exp_filepath in exp_filepaths:
        ...
        if not exp_filepath.find('datasize') == -1:
            current_datasize = int(exp_filepath[exp_filepath.find('datasize')+8:exp_filepath.find('ver')-1])
        else:
        ...
        with open(exp_filepath) as exp_file:
                ...
                if not flag_start:
                    ...
                    if not os.path.exists(exp_filepath) and not os.path.exists(exp_filepath_small):
                        ...
                        with open(exp_filepath_get, 'rb') as exp_handle:
                            ...
                            if not exp_nn == current_nn and not current_nn == '':
                                NN_exp_stats.append({'nn_size': current_nn, 'num_runs': run_counter, 'stats': exp_stats})
                                # reset for new experiment
                                current_nn = exp_nn  # update experiment
                                exp_stats = {}  # reset the experiment stats
                                run_counter = 0  # reset run counter
            ...
            if flag_start:
                NN_exp_stats.append({'nn_size': current_nn, 'num_runs': run_counter, 'stats': exp_stats})
        datasize_exp_stats.append({'data_size': current_datasize, 'all_stats': NN_exp_stats})
...
def plot_pareto_datasize_NN(exp_stats):
    x_data = [str(exp_data['data_size']) for exp_data in exp_stats]  # number of data points in training
    all_y_data = [int(nn_data['nn_size']) for exp_data in exp_stats for nn_data in exp_data['all_stats']]
    seen = set()
    ...
    num_runs = [nn_data['num_runs'] for exp_data in exp_stats for nn_data in exp_data['all_stats'] if
                int(nn_data['nn_size']) in repeated_y_data]
    ...
    # plot dispatch error
    res = [nn_data['stats']['V_dispatch_mean'] for exp_data in exp_stats for nn_data in exp_data['all_stats'] if int(nn_data['nn_size']) in repeated_y_data]
    ...
    # dispatch_colours = 0
    axs[0].scatter(x_data, y_data, s=dispatch_to_plot, alpha=0.75)  # , c=dispatch_colours
    ...
    axs[0].annotate(str(round(max(dispatch) * 1000) / 1000), xy=(x_data[dispatch_max_ind], y_data[dispatch_max_ind]),
                    xytext=(-15, 10), textcoords='offset points')
    # axs[0].set_title('Dispatch error', wrap=True)
    axs[0].set_title("\n".join(wrap('Dispatch error', 12)))

    # plot topology error
    res = [nn_data['stats']['V_topology_mean'] for exp_data in exp_stats for nn_data in exp_data['all_stats'] if
           int(nn_data['nn_size']) in repeated_y_data]
    ...
    axs[1].annotate(str(round(max(topology) * 1000) / 10) + '%', xy=(x_data[topology_max_ind], y_data[topology_max_ind]),
                    xytext=(-15, 10), textcoords='offset points')
    # axs[1].set_title('Topology error', wrap=True)
    axs[1].set_title("\n".join(wrap('Topology error', 12)))

    # plot ineq violation mean
    res = [nn_data['stats']['V_ineq_mag_mean'] for exp_data in exp_stats for nn_data in exp_data['all_stats'] if
           int(nn_data['nn_size']) in repeated_y_data]
    ...
    axs[2].annotate(str(round(max(ineq_mean) * 5680)) + ' kVA', xy=(x_data[ineq_mean_max_ind], y_data[ineq_mean_max_ind]),
                    xytext=(-15, 10), textcoords='offset points')
    # axs[2].set_title('Ineq violation mean', wrap=True)
    axs[2].set_title("\n".join(wrap('Ineq viol mean', 12)))

    # plot ineq violation max
    res = [nn_data['stats']['V_ineq_mag_max'] for exp_data in exp_stats for nn_data in exp_data['all_stats'] if
           int(nn_data['nn_size']) in repeated_y_data]
    ...
    axs[3].set_xticks(axs[3].get_xticks(), axs[3].get_xticklabels(), rotation=45, ha='right')
    # find index of maximum
    ineq_max_max_ind = ineq_max.index(max(ineq_max))
    axs[3].annotate(str(round(max(ineq_max) * 5680)) + ' kVA', xy=(x_data[ineq_max_max_ind], y_data[ineq_max_max_ind]),
                    xytext=(-12, 10), textcoords='offset points')
    # axs[3].set_title('Ineq violation max', wrap=True)
    axs[3].set_title("\n".join(wrap('Ineq viol max', 12)))

    # plot ineq violation at level 1
    res = [nn_data['stats']['V_ineq_num_viol_1'] for exp_data in exp_stats for nn_data in exp_data['all_stats'] if
           int(nn_data['nn_size']) in repeated_y_data]
    ...
    axs[4].set_xticks(axs[4].get_xticks(), axs[4].get_xticklabels(), rotation=45, ha='right')
    # find index of maximum
    ineq_v1_max_ind = ineq_v1.index(max(ineq_v1))
    axs[4].annotate(str(round(max(ineq_v1)/1465*100)) + '%', xy=(x_data[ineq_v1_max_ind], y_data[ineq_v1_max_ind]),
