% updated optimizer for multiple feeder nodes, with the feeders at the end

classdef reconfigure_optimizer
    methods(Static)

        function model_paramzed = get_paramzed_model(networkdata)
...
%                 PG - PL_param == networkdata.Aflow*(Pij - Pji),... % power balance, P
%                 QG - QL_param == networkdata.Aflow*(Qij - Qji)% power balance, Q
...
%             varLims = [
%                 PG_low_param <= PG,...
%                 PG_upp_param(1:networkdata.N-networkdata.numFeeders) >= PG(1:networkdata.N-networkdata.numFeeders),... % feeder PG is unbounded
%                 QG_low_param <= QG,...
%                 QG_upp_param(1:networkdata.N-networkdata.numFeeders) >= QG(1:networkdata.N-networkdata.numFeeders),... % feeder QG is unbounded
%                 [ones(networkdata.N-1,1)*networkdata.vLow^2; 1] <= V,... % includes slack voltage
%                 [ones(networkdata.N-1,1)*networkdata.vUpp^2; 1] >= V % includes slack voltage
%                 ];
            
            load = [
%                 PL(2:end) == PL_param,... % parameterized
%                 QL(2:end) == QL_param,... % parameterized
...
%             ops = sdpsettings('solver','Gurobi','verbose',0); % 0
            ops = sdpsettings('solver','Gurobi','verbose',0,'cachesolvers',1,'savesolveroutput',1);
            model_paramzed = optimizer([powerflow, varLims, load,topological, radiality], objfnc, ops, [PL_param; QL_param; PG_upp_param; PG_low_param; QG_upp_param; QG_low_param], [V;PG;QG;Pij;Qij;Pji;Qji;zij;zji;yij]); % PL;QL; after V; 
            % had [V; QL_feed;...] before
        
        
        %     % FOR DEBUGGING
        %     res = model_paramzed([casedata.PL ; casedata.QL]);
        %     % check against regular optimize
        %     [z_case, zc_case, yaltime_case, solvertime_case, objval_case, error] = reconfigure(networkdata, casedata);% render a parameterized MILP using yalmip's optimizer function
            % vary PL and QL in different runs of the model
            % LinDistFlow power flow model
            % objective function is line losses
...
%         
%             % parameterizations
%             PL_param = sdpvar(networkdata.N-1,1);
%             QL_param = sdpvar(networkdata.N-1,1);
%             PG_upp_param = sdpvar(networkdata.N-1,1);
%             PG_low_param = sdpvar(networkdata.N-1,1);
%             QG_upp_param = sdpvar(networkdata.N-1,1);
%             QG_low_param = sdpvar(networkdata.N-1,1);
%             
%             nosw_inds = setdiff(1:networkdata.M,networkdata.swInds);
%             powerflow = [
%                 networkdata.A(nosw_inds, :)*V == -2*(diag(networkdata.R(nosw_inds))*(Pij(nosw_inds)-Pji(nosw_inds)) + diag(networkdata.X(nosw_inds))*(Qij(nosw_inds)-Qji(nosw_inds))),... % voltage rln
...
%             
%             ops = sdpsettings('solver','Gurobi','verbose',0); % 0
%             model_paramzed = optimizer([powerflow, varLims, load,topological, radiality], objfnc, ops, [PL_param; QL_param; PG_upp_param; PG_low_param; QG_upp_param; QG_low_param], [V;PL;QL;PG;QG;Pij;Qij;Pji;Qji;zij;zji;yij]);
        
        
        %     % FOR DEBUGGING
        %     res = model_paramzed([casedata.PL ; casedata.QL]);
        %     % check against regular optimize
        %     [z_case, zc_case, yaltime_case, solvertime_case, objval_case, error] = reconfigure(networkdata, casedata);
        end % end function


        function [z, zc, yij, solvetime, objval, err, solver_info] = solve_case(model_paramzed, networkdata, casedata) % res

            if length(casedata.PL) == networkdata.N
                time_start = tic;
                [res, err, ~, ~, ~, solver_data] = model_paramzed([casedata.PL(networkdata.numFeeders+1:end);...
                    casedata.QL(networkdata.numFeeders+1:end); casedata.PGUpp; casedata.PGLow; casedata.QGUpp; casedata.QGLow]);
                time_end = toc(time_start);
            else
                time_start = tic;
               [res, err, ~, ~, ~, solver_data] = model_paramzed([casedata.PL;casedata.QL;...
                   casedata.PGUpp; casedata.PGLow; casedata.QGUpp; casedata.QGLow]);
               time_end = toc(time_start); 
            end
            

            solvetime = time_end; 
            solver_info = solver_data.solveroutput.result;
        ...
        %         error('ERROR: Cannot solve reconfiguration. \nYalmip returned %d: %s',err,yalmiperror(err))
                ...
                yij =NaN; solvetime = NaN; objval = NaN;
            else

                % decompose result vector into power variables
                % res = [V;QL_feed;PG;QG;Pij;Qij;Pji;Qji;zij;zji;yij]); % PL;QL; after V
                ind = 1;
                ...
                yij = res(ind:ind+networkdata.numSwitches-1); 
