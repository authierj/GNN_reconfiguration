...
%                 PL(1) >= 0,...  % need positivity of PL and PG for all nodes
            ...
            model_paramzed = optimizer([powerflow, varLims, load,topological, radiality], objfnc, ops, [PL_param; QL_param; PG_upp_param; PG_low_param; QG_upp_param; QG_low_param], [V;QL_feed;PG;QG;Pij;Qij;Pji;Qji;zij;zji;yij]); % PL;QL; after V
...
%             model_paramzed = optimizer([powerflow, varLims, load,topological, radiality], objfnc, ops, [PL_param; QL_param; PG_upp_param; PG_low_param; QG_upp_param; QG_low_param], [V;PL;QL;PG;QG;Pij;Qij;Pji;Qji;zij;zji;yij]);
        ...
        function [z, zc, yij, solvetime, objval, err] = solve_case(model_paramzed, networkdata, casedata) % res

            time_start = tic;
            [res, err] = model_paramzed([casedata.PL ; casedata.QL; casedata.PGUpp; casedata.PGLow; casedata.QGUpp; casedata.QGLow]);
...
%                 QL = res(ind:ind+networkdata.N-1); ind = ind + networkdata.N;
                PL = vertcat(0, casedata.PL);
                QL_feed = res(ind); ind = ind + 1;
                QL = vertcat(QL_feed, casedata.QL);
